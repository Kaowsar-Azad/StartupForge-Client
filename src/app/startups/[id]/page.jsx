  ...
  return (
    <div className="min-h-screen bg-background text-foreground py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto space-y-10">
        ...
        {/* Startup Header card */}
        <div className="bg-white dark:bg-slate-900 border border-divider rounded-3xl p-6 sm:p-10 shadow-2xl flex flex-col sm:flex-row items-center sm:items-start gap-6">
          <img
            src={startup.logo || "/default-startup.png"}
            alt={startup.startup_name}
            className="w-28 h-28 rounded-2xl object-cover border border-divider shadow-md"
          />
          <div className="text-center sm:text-left space-y-2 flex-1">
            <div className="flex flex-col sm:flex-row items-center sm:items-start justify-between gap-4">
              <div>
                <h1 className="text-3xl font-extrabold">{startup.startup_name}</h1>
                <p className="text-blue-500 font-semibold mt-1">{startup.industry}</p>
              </div>
              <span className="bg-blue-500/10 text-blue-550 dark:text-blue-300 text-xs px-3.5 py-1.5 rounded-full font-semibold capitalize">
                Funding: {startup.funding_stage}
              </span>
            </div>
            <p className="text-xs text-default-500 pt-2">
              Founder contact: <span className="text-default-700 font-mono">{startup.founder_email}</span>
            </p>
          </div>
        </div>

        {/* Details & description section */}
        <div className="bg-white dark:bg-slate-900 border border-divider rounded-3xl p-6 sm:p-10 shadow-2xl space-y-6">
          <div>
            <h2 className="text-xl font-bold text-foreground mb-3">About {startup.startup_name}</h2>
        ...
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-foreground">Open Team Opportunities</h2>
          {opportunities.length === 0 ? (
            <div className="bg-default-50 dark:bg-slate-900/50 border border-divider p-8 rounded-3xl text-center text-default-500">
              ...
              {opportunities.map((opp) => (
                <div key={opp._id} className="bg-white dark:bg-slate-900 border border-divider rounded-2xl p-6 flex flex-col justify-between hover:border-blue-500/50 transition-all duration-300 shadow-xl group">
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs text-default-600 bg-default-100 dark:bg-slate-950/40 p-3 rounded-xl">
                      <p><span className="font-semibold text-default-700">Type:</span> {opp.work_type}</p>
                      <p><span className="font-semibold text-default-700">Commit:</span> {opp.commitment_level}</p>
                    </div>
                  </div>
        ...
        <div className="pt-4">
          <Link href="/startups" className="text-default-500 hover:text-foreground transition text-sm">
